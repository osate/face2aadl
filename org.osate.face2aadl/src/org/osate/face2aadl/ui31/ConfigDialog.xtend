 ...
 * DM23-0412
 */
package org.osate.face2aadl.ui31
...
class ConfigDialog extends TitleAreaDialog {
	...
	override protected getInitialSize() {
		new Point(super.initialSize.x, convertVerticalDLUsToPixels(300))
	}
			...
			new Composite(outerComposite, SWT.NONE) => [innerComposite |
				...
				new Group(innerComposite, SWT.SHADOW_NONE) => [otherGroup |
					...
					processIdlButton = new Button(otherGroup, SWT.CHECK) => [button |
						//TODO Change label and don't disable button if we decide to support IDL processing for 3.1
						button.text = "Process IDL files discovered in the project (not available for FACE 3.1)"
						button.layoutData = new GridData(SWT.LEFT, SWT.CENTER, false, false)
	...
	override protected okPressed() {
		...
		//TODO Save processIdl to dialogSettings if we decide to support IDL processing for 3.1
	...
	def private void loadFromSettings() {
		...
		//TODO Load IDL setting if we decide to support IDL processing for 3.1
	}
}